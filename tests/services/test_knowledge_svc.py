...
class TestKnowledgeService:
    ...
    def test_same_fact_different_agents(self, loop, knowledge_svc):
        loop.run_until_complete(knowledge_svc.add_fact(Fact(trait='test', value='demo', score=1,
                                                            collected_by=['thin_airA'], technique_id='T1234',
                                                            source='1234')))
        loop.run_until_complete(knowledge_svc.add_fact(Fact(trait='test', value='demo', score=1,
                                                            collected_by=['thin_airB'], technique_id='T1234',
                                                            source='1234')))
        facts = loop.run_until_complete(knowledge_svc.get_facts(dict(source='1234')))
        assert len(facts) == 1
        assert len(facts[0].collected_by) == 2
